                                                ...
                                                <div class="flex items-center">
                                                    ...
                                                    >
                                                        <img
                                                            ...
                                                            :src="`https://ui-avatars.com/api/?name=${chef.name}&background=random`"
                                                            :alt="chef.name"
                                                        />
                                                    </div>
                                                    <div class="ml-4">
...
const filteredChefs = computed(() => {
    ...
    return props.users
        ...
        .map((chef) => ({
            ...chef,
            recent_recipe:
                recentRecipesMap[chef.id] || "No recent recipes available",
        }));
});
...
const computedStatistics = computed(() => [
    ...
    {
        title: "Total Recipes",
        value: props.statistics[0].total_recipes,
        icon: BookOpenIcon,
    },
    {
        title: "Total Chefs",
        value: props.statistics[0].total_chefs,
        icon: UserGroupIcon,
    },
    {
        title: "Total Admins",
    ...
    confirmModalConfig.value = {
        ...
        onConfirm: () => {
            router.delete(route("admin.deleteUser", { user: userId }), {
                onSuccess: () => {
                    props.users = props.users.filter(
                        (user) => user.id !== userId
                    );
...
const submitUser = () => {
    if (editingUser.value) {
        if (
            ...
            showAlert(
                "Once promoted to Admin, users cannot be demoted.",
                "warning"
            );
            return;
        }
...
const refreshActivityLogs = () => {
    router.get(
        route("admin.getActivityLogs"),
        {},
        {
            preserveState: true,
            preserveScroll: true,
            onSuccess: (page) => {
                activityLogs.value = page.props.logs;
