    ...
    <div class="flex flex-col lg:flex-row min-h-screen bg-gray-100">
      ...
      <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-100 custom-scrollbar">
        <div class="container mx-auto px-4 lg:px-6 py-8">
          ...
          <!-- Staff Recent Tribe -->
          <section id="recent-tribes" class="mb-12">
            <div class="flex justify-between items-center mb-6">
              <h2 class="text-3xl font-bold text-gray-800">Recent Staff Tribe</h2>
            </div>
                ...
                <table class="w-full">
                  <thead>
                    <tr class="bg-gray-50 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                      ...
                      <th class="px-6 py-3">Recent Tribe</th>
                    </tr>
                  </thead>
                  <tbody class="divide-y divide-gray-200">
                    <tr v-for="staff in filteredStaffs" :key="staff.id" class="hover:bg-gray-50">
                      <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center">
                          <div class="flex-shrink-0 h-10 w-10">
                            <img class="h-10 w-10 rounded-full" :src="`https://ui-avatars.com/api/?name=${staff.name}&background=random`" :alt="staff.name" />
                          </div>
                          <div class="ml-4">
          ...
          <section id="users" class="mb-12">
            ...
            <div class="grid grid-cols-1 gap-6 mb-10">
              <div v-for="role in ['admin', 'staff', 'viewer']" :key="role" class="bg-white p-4 rounded-lg shadow-md overflow-x-auto">
                    ...
                    <tr v-for="(user, index) in groupedUsers[role]" :key="user.id">
                      ...
                      <td class="px-6 py-4 whitespace-nowrap">
                        <span :class="{
                          ...
                          'bg-blue-100 text-blue-800': getRole(user.role_id) === 'Staff',
          ...
          <section id="tribes" class="mb-12">
            <div class="flex justify-between items-center mb-6">
              <h2 class="text-3xl font-bold text-gray-800">Tribe Overview</h2>
            </div>
            <div class="bg-white rounded-xl shadow-md overflow-hidden">
              <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                  <thead class="bg-gray-50">
                    <tr>
                      <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tribe Name</th>
                  ...
                  <tbody class="bg-white divide-y divide-gray-200">
                    <tr v-for="tribe in sortedTribes" :key="tribe.tribe_id" class="hover:bg-gray-50">
                      <td class="px-6 py-4 whitespace-nowrap">{{ tribe.tribe_name }}</td>
                      <td class="px-6 py-4 whitespace-nowrap">
                        <span :class="getCategoryClass(tribe.category_name)" class="px-3 py-1 rounded-full text-xs font-medium">
                          {{ tribe.category_name }}
                        </span>
                      </td>
                      <td class="px-6 py-4 whitespace-nowrap">{{ tribe.staff_name }}</td>
                      <td class="px-6 py-4 whitespace-nowrap">{{ formatDate(tribe.created_at) }}</td>
                      <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <button
...
const filteredStaffs = computed(() => {
  ...
  return props.users
    ...
    .map(staff => ({
      ...staff,
      recent_tribe: recentTribesMap[staff.id] || 'No recent tribes available',
    }));
});
  ...
  confirmModalConfig.value = {
    ...
    onConfirm: () => {
      router.delete(`/admin/tribes/${tribe.tribe_id}`, {
        ...
        preserveScroll: true,
        onSuccess: () => {
          tribes.value = tribes.value.filter(r => r.tribe_id !== tribe.tribe_id);
...
const computedStatistics = computed(() => [
  ...
  { title: "Total Tribes", value: props.statistics[0].total_tribes, icon: GlobeAltIcon },
  { title: "Total Staffs", value: props.statistics[0].total_staffs, icon: UserGroupIcon },
